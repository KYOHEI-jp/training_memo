          ...
          if (snapshot.hasData) {
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                String formattedDate = DateFormat('yyyy/MM/dd HH:mm').format(record.createdAt ?? DateTime.now());
                return Card(
                  child: ListTile(
                    title: Text('${record.part} - ${record.exercise}'),
                    subtitle: Text('重量: ${record.weight} kg, 回数: ${record.reps}回, 日時: $formattedDate'),
                  ),
                );
              },
            );
